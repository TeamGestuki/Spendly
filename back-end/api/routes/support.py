...
SMTP_PORT = 587
SMTP_USERNAME = "teamgstspendly@gmail.com"
SMTP_PASSWORD = "8524563tgm"


def send_support_email(report: SupportReport):
...
async def get_my_support_reports(
    status: Optional[SupportReportStatus] = Query(None, description="Filtrar por estado"),
    ...
    reports = query.order_by(SupportReport.created_at.desc()).offset(offset).limit(limit).all()
...
async def get_support_report(
    ...
    if session.user_id != report.user_id:
        ...
        is_admin = db.query(UserSession).filter(
            UserSession.id == session.id
        ).first() is not None

        if not is_admin:
            logger.warning(f"User {current_user.id} attempted to access report {report_id} belonging to user {report.user_id}")
            raise HTTPException(
...
async def update_support_report(
    ...
    # Check if current user is admin (using session)
    is_admin = db.query(UserSession).filter(
        UserSession.id == session.id
    ).first() is not None

    if not is_admin:
        logger.warning(f"User {current_user.id} attempted to update report {report_id} without admin privileges")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
